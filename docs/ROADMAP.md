...
- [ ] **After merge**: OTA workflow goes RED by design (fingerprint moved, no build serves the new runtime) until the new `preview` + `preview-simulator` EAS builds exist → install sim build → full audio regression (song-search tap-preview, detail progress + scrub, pause/resume, delete-stops-playback, onboarding preview, background-pause) + gate check (raise staging floor above 23 → update screen; reset) → TestFlight submit (beta-first; prod binary only when the owner calls the beta ready).
