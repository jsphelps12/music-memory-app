...
- [x] **WS-I — E2E smoke flow GREEN locally** (2026-07-30): full pass on simulator — sign-in → create → live Apple Music search → save → force-quit → relaunch → moment on first paint. Debugging findings baked into `.maestro/smoke.yaml` comments: system dialogs blank the whole XCUITest hierarchy (handle *before* waits, with optional waits — the Apple Account alert is OS-throttled and nondeterministic); the floating "+" has a broken accessibility frame (element taps no-op; flow uses the empty-state CTA the seed guarantees); seed must set `onboarding_completed`; `hideKeyboard` unreliable. **CI wiring complete (2026-07-31):** repo secrets set, `preview-simulator` build 23 on EAS, and a green run on main at `efcfda8` (run 30608171462) — the exact SHA the promote gate will query. WS-I fully done; the weekly Monday run now guards against rot. **Maestro pinned to 2.7.0 (2026-08-07):** the installer is unpinned by default and picked up 2.8.0 — published 2026-07-31 12:04 UTC, six hours *after* the last green run — which broke every run from the 2026-08-03 cron onward. 2.8.0 starts typing before the keyboard its own tap raised has settled, so the flow's **first** `inputText` lands only its first character; the email field held `"e"`, Supabase rejected the sign-in, and the flow died 20 steps later asserting `tab-create`. Both failing runs were identical, so this was never flake, and the misleading distance between symptom and cause is why the flow now asserts the email field round-tripped right where it is typed. Two lessons: an unpinned test runner cannot tell you the app broke, and the gate had been dark since 2026-07-31 — nothing after sign-in ran, so the whole 2026-08-01/02 wave (pills, `albums.tsx` rename, ConfirmSheet, expo-audio) went unexercised. **Second blocker, found the moment sign-in worked again:** iOS fires its one-time "slide your finger across the letters" keyboard tutorial at the first STANDARD keyboard (sign-in's email-address keyboard has no QuickPath, so it lands on song search) and swallows the query — the native field showed "Bohemian Rhapsody" while `onChangeText` never fired, so the controlled input never re-rendered, the screen sat on its idle hint, and no search ran. It is undismissable from the flow: it renders in the keyboard's own process, so unlike the Apple Account and Save Password alerts it never enters the app's accessibility hierarchy, yet it still blanks everything behind it (the failure hierarchy held only the search input and the status bar). Suppressed in `e2e.yml` with `defaults write … DidShowContinuousPathIntroduction` on the fresh simulator. Both text fields now assert their value round-tripped, because "the query never reached the app" and "the catalog returned nothing" are indistinguishable downstream and only the second is a real failure.
...
## Build 23 — in flight 2026-08-02 (the fingerprint-moving binary)

- [x] **expo-audio swap** *(code shipped; regression pass pending the new binary)*: expo-av removed entirely; `contexts/PlayerContext.tsx` preview fallback + `app/onboarding/moment-preview.tsx` on `createAudioPlayer`/`.play()/.pause()/.remove()/.seekTo(seconds)` + `addListener("playbackStatusUpdate")`; positions now natively seconds (the /1000s are gone); `setAudioModeAsync({playsInSilentMode})` key rename; createAudioPlayer being synchronous also retired the onboarding cancelled-flag dance. Provider playback untouched.
- [x] **Min-build gate** *(code shipped, inert)*: `app_config` table (migration `20260802100000`, applied both projects, anon-readable, no write policies — value changes via dashboard/service only) seeded `min_supported_build = 1`; `lib/minBuildGate.ts` checks `expo-application` nativeBuildVersion against the floor on launch with **fail-open as the invariant** (4 vitest cases pin it: only positive knowledge of current < min blocks; every unparseable/missing input runs the app); `UpdateRequiredScreen` (serif voice, App Store button, deliberately no dismiss) replaces the tree in the root layout when tripped. Future teardowns = raise one config value.
- [x] **Post-merge verification** *(2026-08-02)*: the OTA workflow went red exactly as designed — the fingerprint gate reported *"No installed binary would receive this update"* (commit runtime `20d72289…` vs newest preview build 22 serving `eedd91e9…`) and refused to publish into the void. `preview-simulator` build cut, installed, verified:
  - **Audio (expo-audio)**: autoplay on moment detail; position/duration render in **seconds** (`0:01 / 0:29` — the ms-vs-seconds trap would have shown `0:00` or garbage); **scrub** seeks proportionally (tapped ~87% → `0:26 / 0:29`, proving `seekTo(seconds)`); `didJustFinish` cleans up (progress clears, button returns to Play). Runtime log scan shows **zero expo-audio/AudioPlayer errors** — the only failures are `MPMusicPlayerController is not available on the simulator`, i.e. the Apple Music provider, which is untouched by this change and **cannot be verified outside a device** (defer to the device regression checklist).
  - **Min-build gate**: staging floor 1 → 2, relaunch → "time for an update" screen renders (serif voice, App Store button, no dismiss); floor reset to 1, relaunch → app returns to normal. Prod floor confirmed still 1 throughout (never touched).
