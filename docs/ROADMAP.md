...
11. **Add a moment from inside an album → auto-added to that album** 🟢 *(owner request 2026-07-31, Wave 3)* — from `app/album/[id].tsx`, a create entry point that carries the album context: pass `collectionId` to `/create` (route param), preselect it in the album picker state, and write the `collection_moments` row on save via the existing `lib/saveMoment.ts` path. Respect member-vs-owner role (members can already contribute to shared albums).

**Wave plan** (Waves 1–2 shipped 2026-07-31; each wave = one branch → PR → merge → OTA → simulator verification pass on the preview-simulator build):
- **Wave 3 — features & hygiene**: item 2 (avatar full-size, dual-size upload), item 11 (create-from-album), item 7 (`guest_name`), item 8 (edge functions under tsconfig/eslint). All JS-only → OTA to build 22.
- **Wave 4 — structural**: item 4 (multi-mood migration + dual-write) · expo-av → expo-audio assessment (item 9 — fingerprint-moving, batch with next binary).
...
7. **`guest_name` silently dropped** 🟢 *(Wave 3)* — wedding guests pick a name on every page; it's discarded. Wire it through or remove the field.
8. **`supabase/functions` outside tsconfig/eslint** 🟢 *(Wave 3)* — the notification edge function is not type-checked. Add a scoped tsconfig or include path.
