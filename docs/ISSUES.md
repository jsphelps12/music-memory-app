...
| 3 | Prefetch userId mismatch — data leak on account switch | High | Fixed | `lib/timelinePrefetch.ts` — replaced `_promise` with `_prefetch: { promise, userId }`; consume validates userId and returns null on mismatch; clearTimelineCache also clears in-flight prefetch. |
...
| 11 | Deep link handler stale closure | Low | Fixed | `hooks/useDeepLinkHandler.ts` — added `handleInviteCode` to clipboard `useEffect` dep array. Clipboard is cleared on first read so re-runs are safe. |
