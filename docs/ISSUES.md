...
| 4 | Auth listener writes state after unmount | High | Open | `contexts/AuthContext.tsx` — `fetchProfile` can fire after component unmounts if `getSession()` resolves late. Fix: mounted ref guard. |
| 5 | Auth state change race condition | High | Open | `contexts/AuthContext.tsx` — `fetchProfile` not awaited inside `onAuthStateChange`; rapid sign-out/sign-in could cross profile data between users. |
| 6 | Silent push token registration failure | High | Open | `app/_layout.tsx` — `.catch(() => {})` swallows all errors silently. Users may never receive notifications with no indication why. Fix: log to Sentry + PostHog. |
